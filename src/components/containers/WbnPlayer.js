...
const WbnPlayer = ({ match, history, location }) => {
  ...
  const savedSatete = JSON.parse(localStorage.getItem(`${videos.playlistId}`));

  const [state, setState] = useState({
    videos: savedSatete ? savedSatete.videos : videos.playlist,
    activeVideo: savedSatete ? savedSatete.activeVideo : videos.playlist[0],
    nightMode: savedSatete ? savedSatete.nightMode : true,
    playlistId: savedSatete ? savedSatete.playlistId : videos.playlistId,
    autoplay: false
  });
